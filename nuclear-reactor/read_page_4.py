...
if __name__ == '__main__':
    ...
    with open('nuclear-reactor/index/index_4.json', 'r') as f:
        index = json.load(f)
    ...
    def save():
        print(words)
        with open('nuclear-reactor/output/words_4.txt', 'w') as f:
            json.dump(words, f)
        f.close()

        print(types)
        with open('nuclear-reactor/output/types_4.txt', 'w') as f:
            json.dump(types, f)
        f.close()
        
        print(total_cost)
        with open('nuclear-reactor/output/total_cost_4.json', 'w') as f:
            json.dump(total_cost, f, indent=4)
        f.close()

        json_data = json.dumps(info, indent=4)
        with open('nuclear-reactor/output/info_4.json', 'w') as f:
            f.write(json_data)
        f.close()

        end_time = time.time()
        print(f"Time: {end_time - start_time} seconds.")
        with open('nuclear-reactor/output/time_4.json', 'w') as f:
